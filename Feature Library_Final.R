...
form <- y ~ .^2 + .^3            # This will tell the next line to add all pairwise / three way interactions between predictors
...
#flib2 <- as.matrix(flib_function(y2, Xtrain2017))
...
#colnames(flib2) = make.names(make.unique(colnames(flib2)), unique=TRUE)
colnames(flibtest) = make.names(make.unique(colnames(flibtest)), unique=TRUE)

## Glmnet-Lasso for feature/model selection
## Using family="binomial" to do the logistic regression because the response is binary
...
# glmnet2 <- cv.glmnet(flib2, y2, type.measure = 'mse', nfolds = 10, alpha = 1, family = "gaussian", nlambda = 10)  
...
#variables2 <- variables2[variables2 %ni% "(Intercept)"]
lassoXtrain2017 = flib[,variables]                    # The remaining features, selected from LASSO
#lassoXtrain2017_2 = flib2[,variables2]                # The remaining features of the 2 yr, selected from LASSO
...
#ridge.cv.2017.2yr = cv.glmnet(y = as.numeric(ytrain2017_2yr), x = as.matrix(lassoXtrain2017_2), alpha = 0, family = "binomial")
# plot(ridge.cv.2017.1yr)
ridge.coefsfull.2017.1yr = coef(ridge.cv.2017.1yr, s = "lambda.min")  # Get the model coefficients
#ridge.coefsfull.2017.2yr = coef(ridge.cv.2017.2yr, s = "lambda.min")  
ridge.predictions = signif(predict.cv.glmnet(ridge.cv.2017.1yr, newx = as.matrix(lassoXtest2017), s = "lambda.min", type = "response"),4)
#ridge.predictions2 = signif(predict.cv.glmnet(ridge.cv.2017.2yr, newx = as.matrix(lassoXtest2017), s = "lambda.min", type = "response"),4)
...
#elastic.cv.out2 = cv.glmnet(y = as.numeric(ytrain2017_2yr), x = as.matrix(lassoXtrain2017_2), alpha = 0.5, family = "binomial")

elastic.min.out = coef(elastic.cv.out, s = elastic.cv.out$lambda.min)
#elastic.min.out2 = coef(elastic.cv.out2, s = elastic.cv.out2$lambda.min)
...
#elastic.train.min.predictions2 = predict.cv.glmnet(elastic.cv.out2, newx = as.matrix(lassoXtrain2017_2), s = "lambda.min", type = "response")
#elastic.test.min.predictions2 = predict.cv.glmnet(elastic.cv.out2, newx = as.matrix(lassoXtest2017), s = "lambda.min", type = "response")
...
#Xtest2017$risk2017.2yr.EN = as.numeric(elastic.test.min.predictions2)
#Xtest2017$risk2017.2yr = as.numeric(ridge.predictions2)
...
# We can hand craft a loss function to "go easy" on models which predict mk's a year in advance too, once we do multi-year windows
...
everything2017 = Xtest2017 %>%  select(country_name, sftgcode, COWcode, risk2017.1yr, risk2017.1yr.EN, risk2017.2yr, risk2017.2yr.EN) %>% arrange(desc(risk2017.1yr))
colnames(everything2017) = c("country","SFTGcode","COW","risk_in_2017_ridge","risk_in_2017_EN","risk_in_2017_2018_ridge","risk_in_2017_2018_EN")
View(everything2017)
...
results2<-variables2
write.table(results2, paste0(getwd(), "/results/lassoresults2.csv"), sep = ",", row.names = F, col.names = F, quote = F, append = F)